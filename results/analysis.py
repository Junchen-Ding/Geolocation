...
def analyze_samples(df, n=5, dataset_name='dataset') :
    ...
    print(highest_errors[['RelativePath', 'LAT', 'LON', 'Latitude_diff', 'Longitude_diff', 'Geodesic_distance_km']])
    print(f"\nLowest errors for {dataset_name}:")
    print(lowest_errors[['RelativePath', 'LAT', 'LON', 'Latitude_diff', 'Longitude_diff', 'Geodesic_distance_km']])
...
for index, row in combined_samples.iterrows() :
    print(
        f"Sample Type: {row['SampleType']}, Image: {row['RelativePath']}, Geodesic Distance: {row['Geodesic_distance_km']:.2f} km")
